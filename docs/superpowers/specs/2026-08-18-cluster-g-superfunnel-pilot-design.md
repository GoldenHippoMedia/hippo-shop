...
- **`referralUrl`** = `?referral_url=` when present, **and omitted entirely otherwise** (`session.service.ts:138`, `:143` — a spread guard, so the key is absent rather than empty). Cluster F's `document.referrer` fallback (`url-params.ts:95`) is **removed from the session POST body**. The reference never derives this field from `document.referrer` on this path; its only referrer-derived `referralUrl` is in the funnel-event payload (`funnel-event.service.ts:176-180`), a different call with a different shape.
  ...
  Keeping the fallback would be actively harmful, not merely divergent. `affParameters` is destructive-on-write and D4 posts on every page load, so on the second page view `document.referrer` is the *previous internal page* — and the POST would overwrite the true ad referrer that the reference captured server-side (`cid/router.ts:186-189` sets `referral_url` from the raw `Referer` header). That is precisely the class of silent attribution corruption Cluster G exists to fix. [**Premise superseded** later in this run — see the note on the empty-values bullet below. The mechanism described here cannot happen: `gh-service-utils@1.11.x` merges as `{ ...incoming, ...req.session[brand] }`, so a second-page-view POST can never overwrite a `referralUrl` the commerce session already holds. **The removal itself still stands, on its own footing and for a stronger reason.** Because storage is per-key first-write-wins, a `document.referrer` value carried on a *first* page view would durably poison `referralUrl` — and no later POST, however correct, could repair it. The failure is not "overwritten later" but "wrong from the first write and permanent for the life of the session". The separate real risk this reasoning obscured: the commerce session and the funnel's own `referral_url` (captured from the raw `Referer` header in a different system) can disagree permanently, and the commerce side has no mechanism to reconcile them.]
- **Empty values are omitted, never sent as `""`.** The `affParameters` contract is destructive-on-write: the backend treats every key present as authoritative, so `utmSource: ''` blanks a real stored value (`analytics-and-attribution.md:147-149`). [**Premise superseded** later in this run: the destructive-on-write model was traced and executed against `gh-service-utils@1.11.2` and is wrong in both halves. (1) `setSessionParameters` merges `{ ...incoming, ...req.session[brand] }` — the stored session is spread **second**, so storage is per-key **first-write-wins**; an existing value always survives and the library's own JSDoc states "params are not updated if they are already set". (2) Blanks are discarded, not applied: `nullCheckAffParam` maps `''` and `'   '` to `''`, `truncate` maps that to `undefined`, and `undefined` is filtered out before the merge — so an empty string, a whitespace-only string and an absent key are indistinguishable, and `utmSource: ''` cannot blank anything. The cited `analytics-and-attribution.md:147-149` lives in a third repo and asserts the same falsehood, so this decision was never checkable from here. **Omitting empty values is kept regardless** — it is smaller on the wire and leaves one unambiguous form for "no value". Two further facts this premise omitted: every stored value is truncated server-side (255, or 18 for `offId`/`affId`), and the seven raw click-id fields added above are not among the 18 keys the session persists, so they are discarded on write. See `packages/sdk/SPEC.md` § "`POST <base>/public/v1/session`" for the corrected contract.]
...
3. **Destination URL.** Retrieve via a direct SOQL query (a second trip to Salesforce for the same record) and emit as `url`. Interim by explicit decision; the managed-package field is a later cluster.
   ...
   **Field:** `TouchCRBase__Full_Generic_URL__c`.

   **Copy the `Campaign.service.ts:44-70` pattern verbatim** — it is the only hand-built SOQL in the codebase and it carries two guards this query needs just as much:

   - `SALESFORCE_ID_PATTERN` (`/^[a-zA-Z0-9]{15}([a-zA-Z0-9]{3})?$/`, `Campaign.service.ts:13`) validated against **both** the destination id and `this.brand.id` before interpolation, because both land in the same `WHERE`.
   - Brand scoping via `TouchCRBase__Business_Unit__c = '<brand.id>'`. Without it a destination id from another brand would resolve, breaking the cross-brand 404 guarantee in `SPEC.md`.

   Executed through `salesforceService.runQueryViaRest(query, this.brand.accessToken)`. `HippoShopService` already holds `CommerceBrand` (`HippoShop.service.ts:76-80`) and instantiates collaborators per call, so no constructor change is needed.

   **The lookup must never fail the destination response.** A SOQL error or timeout logs and yields `url: null` — indistinguishable to the client from "Salesforce has none", which is the correct degradation given D7's fallback chain and Goal 8. Making a supplementary field a hard dependency would turn one flaky Salesforce call into six failed requests per offer-selector page load. Issue it **in parallel** with the primary destination fetch, not sequentially, for the same reason. The sObject **API name** is the one remaining input — the field is confirmed, but whether the object is `TouchCRBase__Destination__c` (inferred from the lookup field of that name on `Campaign`) must be confirmed before this task runs. Everything else here is complete.
...
5. **What is the Destination sObject's API name?** The URL field is confirmed as `TouchCRBase__Full_Generic_URL__c`. The object is inferred to be `TouchCRBase__Destination__c` from the lookup field of that name on `Campaign` (`Campaign.service.ts:52`), but destinations reach this service through an Apex REST route rather than SOQL, so the object name is not verifiable from either repo. A wrong object name is a runtime `INVALID_TYPE` on every destination request, so confirm before W2.3 runs.
