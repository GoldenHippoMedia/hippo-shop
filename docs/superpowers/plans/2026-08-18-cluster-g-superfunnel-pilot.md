...
- **`affParameters` is first-write-wins per key.** [**Premise corrected post-run.** This constraint originally read: "`affParameters` is destructive-on-write. Every key present overwrites stored attribution. Empty values must be **omitted**, never sent as `""`." Both halves were wrong about `gh-service-utils@1.11.2`, which is what commerce runs. `nullCheckAffParam` maps a blank or whitespace-only value to `""`, `truncate` maps `""` to `undefined`, and `undefined` keys are filtered out — so a blank, a whitespace-only value and an absent key are indistinguishable, and none can clear a stored value. The merge is `{ ...incoming, ...storedSession }`, so an **existing** value wins; the method's own JSDoc says "params are not updated if they are already set". Omitting blanks remains the right wire form — it is just smaller and unambiguous, not a guard against data loss. The real consequence is the opposite of the original: a later POST can fill a key that is missing but can never correct one that is already wrong, so the **first** POST carrying a key is the one that sticks. See `packages/sdk/SPEC.md` § "POST /public/v1/session".]
