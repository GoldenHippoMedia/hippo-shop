...
7. **RESOLVED — the endpoint now exists, but is not yet merged or deployed.** It was implemented as part of this cluster (GH-Commerce-Service PR #350, `feat/cluster-g-hippo-shop-session-destination-url` → `prerelease`, commit `491a851`): a public `POST /v1/session` on `HippoShopController`, which is already `disableAuth = true` and brand-scoped, delegating to the existing `SessionService.getSession`. Configure the Kong route only **after that PR merges and deploys**. Original note follows: The SDK ships the caller; the handler is Workstream 2 work on `GH-Commerce-Service@prerelease`. `HippoShopService` has no session method today. Configure the route only after the endpoint answers.
...
16. ~~**`X-GH-Event-Id` is not sent by the shipped SDK**~~ — **RESOLVED.** The SDK now mints and sends it: `emitPageView` attaches it per event (`packages/sdk/src/events.ts`, commit `11f4b89`). Allowlisting it in `cors.headers` describes current traffic, not pre-provisioning. Original note follows: — it exists only in a docblock, and no code mints an event id. Allowlisting it in `cors.headers` is deliberate pre-provisioning, not a description of current traffic.